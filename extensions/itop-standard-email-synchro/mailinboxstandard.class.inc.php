...
class MailInboxStandard extends MailInboxBase
{
	public static function Init()
	{
		...
		MetaModel::Init_SetZListItems('details', array(
			'col:col0' => array(
					'fieldset:MailInbox:Server' => array('server', 'login', 'password', 'protocol', 'ssl','port', 'mailbox', 'active', 'trace'),
...
EOF
	...
	{		
		...
		switch($oSet->Count())
		{
			...
			switch($this->Get('unknown_caller_behavior'))
			{
				...
				$oCaller->Set('name', $oEmail->sCallerName);
				$oCaller->Set('first_name', ' ');
		...
		// Case of a closed ticket, trigger an error in order to handle it as an error
		if (is_object($oTicket)) {
			// Check the ticket status
			// if closed, trigger an error and do not continue
			if ($oTicket->Get('status') == 'closed') {
				$this->Trace('Failed to update a closed ticket.');
				$oRawEmail = $oSource->GetMessage($index);
				$this->HandleError($oEmail, 'failed_to_update_closed', $oRawEmail);
				return null;
			}
		}
	...
	public function HandleError($oEmail, $sErrorCode, $oRawEmail = null, $sAdditionalErrorMessage = '')
	{
		...
		switch($sErrorCode)
		{
			...
			case 'failed_to_update_closed':
				$sSubject = '[iTop] Impossible de mettre à jour un ticket clos - '.$oEmail->sSubject;
				$sBody = "<p>L'email suivant (voir pièce jointe) vient du demandeur : (".$oEmail->sCallerEmail.").<br/>\n";
				$sBody .= "Et n'a pas pu être traîté car le ticket de référence est clos.</p>\n";
				$sBody .= $sLastAction;
